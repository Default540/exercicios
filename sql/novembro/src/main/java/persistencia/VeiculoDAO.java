...
//       
    ...
    public void inserir(Veiculo veiculo) throws SQLException {
        String sql = "INSERT INTO veiculo(ano,placa,foto) VALUES ('" +veiculo.getAno()+"','"+veiculo.getPlaca()+"','"+veiculo.getFoto()+"');";
    ...
    public void atualizar(Veiculo veiculo) throws SQLException{
        String sql = "UPDATE veiculo set placa = '"+veiculo.getPlaca()+"', ano = '"+veiculo.getAno()+ "', foto = '"+ veiculo.getFoto()+"' WHERE id = '"+veiculo.getId() + "';";
        Connection conexao = new ConexaoPostgreSQL().getConexao();
        conexao.prepareStatement(sql).executeUpdate();
        conexao.close();
    }
